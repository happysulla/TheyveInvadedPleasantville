...
namespace PleasantvilleGame
{
    ...
    {
        #region Fields (myGameEngine, myGameInstance, myButtons, myMovingButton,myRectangleSelection, myIsZebulonTerritoriesVisible )
        ...
        private List<ITerritory> myTerritoriesCombatForAlien = new List<ITerritory>();
        private List<ITerritory> myTerritoriesCombatForTownsperson = new List<ITerritory>();
        ...
        public bool IsAlien
        {
            get { return myIsAlien; }
        }
        #endregion

        #region Constructor(IGameEngine ge, IGameInstance gi, bool isServer, bool isAliene)
        public GameViewerWindow(IGameEngine ge, IGameInstance gi, bool isServer, bool isAlien )
        {
            InitializeComponent();
            ...
            if (true == myIsAlien)
                myTextBoxEntry.Foreground = Utilities.theAlienControlledBrush;
            else
                myTextBoxEntry.Foreground = Utilities.theTownControlledBrush;

            myTimer.Interval = ANIMATE_SPEED*1000 + 1000;
            myTimer.Tick += new EventHandler( this.TimerElasped );

            if (true == isAlien)
                this.BorderBrush = Utilities.theAlienControlledBrush;
            else
                this.BorderBrush = Utilities.theTownControlledBrush;
            ...
            myTerritories = Territory.CreateTerritories();
            foreach (ITerritory t in myTerritories)
            {
                if (0 < t.Points.Count)
                {
            ...
            foreach (IMapItem person in gi.Persons)
            {
                ...
                Canvas.SetLeft(b, person.Location.X - Utilities.theXOffset);
                Canvas.SetTop(b, person.Location.Y - Utilities.theYOffset);
                ...
                b.Width = 50.0;
                b.IsEnabled = true;
                MapItem.SetButtonContent(b, person, myIsAlien);
                myButtons.Add(b);
            ...
            Utilities.theTownControlledBrush.Color = Color.FromArgb(0xFF, 0x33, 0xAA, 0x33);
            Utilities.theAlienControlledBrush.Color = Color.FromArgb(0xFF, 0xFF, 0xD5, 0x00);  // 0xFFFFD500
            Utilities.theSkepticalBrush.Color = Color.FromArgb(0xFF, 0xF2, 0xDE, 0x9B);
            Utilities.theWaryBrush.Color = Color.FromArgb(0xFF, 0x87, 0xE5, 0x87);
        ...
        #region void UpdateView(ref IGameInstance gi, GameAction action)
        public void UpdateView(ref IGameInstance gi, GameAction action)
        {
            ...
            switch (action) // Perform acton based on the current next action.
            {
                ...
                case GameAction.AlienInitiateCombat:
                    if ((true == myIsCombatInitiatedForAlien) && (true == myIsAlien))
                    {
                        UpdateCanvas(gi);
                ...
                case GameAction.TownspersonInitiateCombat:
                   if ((true == myIsCombatInitiatedForTownsperson) && (false == myIsAlien))
                    {
                        UpdateCanvas(gi);
                ...
                case GameAction.TownspersonCompletesCombat:
                    ...
                    UpdateViewState(gi);
                    myIsCombatInitiatedForTownsperson = false;
                    StringBuilder sb2 = new StringBuilder("UpdateView():TownspersonCompletesCombat: "); sb2.Append( myIsAlien.ToString() ); sb2.Append("myIsCombatInitiatedForTownsperson=false"); 
                ...
                case GameAction.AlienCompletesCombat:
                    ...
                    UpdateViewState(gi);
                    myIsCombatInitiatedForAlien = false;
                    StringBuilder sb3 = new StringBuilder("UpdateView():AlienCompletesCombat: "); sb3.Append( myIsAlien.ToString() ); sb3.Append("myIsCombatInitiatedForAlien=false"); 
                ...
                case GameAction.ShowEndGame:
                    ...
                    else
                    {
                        ...
                        myLabelWinner.Foreground = Utilities.theTownControlledBrush;
                    }
                    UpdateCanvas(gi);
                    UpdateViewState(gi);
                    break;
                ...
                case GameAction.ShowAlien:
                    ...
                    stacks.AssignPeople(gi.Persons, myIsAlien);
                    foreach (Stack stack in stacks)
                    {
                        foreach (IMapItem mi in stack.MapItems)
                        {
                            if (true == mi.IsAlienKnown)
                            {
                                ...
                                if (null != b)
                                    MapItem.SetButtonContent(b, mi, myIsAlien);
        ...
        #region void UpdateActionPanel(IGameInstance gi, bool isOkButtonDisplayed)
        public void UpdateActionPanel(IGameInstance gi, bool isOkButtonDisplayed)
        {
            ...
            switch (myLeftMapItemsInActionPanel.Count)
            {
                ...
                case 1:
                    MapItem.SetButtonContent(myButton1, myLeftMapItemsInActionPanel[0]);
                ...
                case 2:
                    MapItem.SetButtonContent(myButton1, myLeftMapItemsInActionPanel[0]);
                    MapItem.SetButtonContent(myButton2, myLeftMapItemsInActionPanel[1]);
                    myButton1.Visibility = Visibility.Visible;
            ...
            switch (myRightMapItemsInActionPanel.Count)
            {
                case 0:
                    break;
                case 1:
                    MapItem.SetButtonContent(myButton4, myRightMapItemsInActionPanel[0]);
        ...
        #region void UpdateActionPanelButtons(IGameInstance gi)
        void UpdateActionPanelButtons(IGameInstance gi)
        {
            if (Visibility.Visible == myButton1.Visibility)
            {
                if (0 < myLeftMapItemsInActionPanel.Count)
                {
                    ...
                    if (null != mi)
                        MapItem.SetButtonContent(myButton1, mi, myIsAlien);
                }
            }
            if (Visibility.Visible == myButton2.Visibility)
            {
                if (1 < myLeftMapItemsInActionPanel.Count)
                {
                    ...
                    if (null != mi)
                        MapItem.SetButtonContent(myButton2, mi, myIsAlien);
                }
            }
            if (Visibility.Visible == myButton3.Visibility)
            {
                if (2 < myLeftMapItemsInActionPanel.Count)
                {
                    ...
                    if (null != mi)
                        MapItem.SetButtonContent(myButton3, mi, myIsAlien);
                }
            }
            ...
            if (Visibility.Visible == myButton4.Visibility)
            {
                if (0 < myRightMapItemsInActionPanel.Count)
                {
                    ...
                    if (null != mi)
                        MapItem.SetButtonContent(myButton4, mi, myIsAlien);
                }
            }
            if (Visibility.Visible == myButton5.Visibility)
            {
                if (1 < myRightMapItemsInActionPanel.Count)
                {
                    ...
                    if (null != mi)
                        MapItem.SetButtonContent(myButton5, mi, myIsAlien);
                }
            }
            if (Visibility.Visible == myButton6.Visibility)
            {
                if (2 < myRightMapItemsInActionPanel.Count)
                {
                    ...
                    if (null != mi)
                        MapItem.SetButtonContent(myButton6, mi, myIsAlien);
                }
            }
        }
        #endregion

        #region void UpdateViewMovement(IGameInstance gi)
            ...
            try
            {
                ...
                foreach (IMapItemMove mim2 in gi.MapItemMoves)
                {
                    ...
                    if (null == alreadyMovedMapItem)
                    {
                        ...
                        sb1.Append(mi.Name);
                        Logger.Log(LogEnum.LE_MOVING_MAPITEM_COUNT, sb1.ToString());
                    }
        ...
        #region void UpdateViewState( IGameInstance gi )
        private void UpdateViewState(IGameInstance gi)
        {
            ...
            switch( gi.GamePhase )
            {
                case GamePhase.Conversations:
                    ...
                    {
                        if (true != myIsAlien || false != myConversationsCompleted)
                        {
                            break;
                        }
                ...
                case GamePhase.Influences:
                    ...
                    {
                        if ((true == myIsAlien) && (false == myInfluencesCompleted))
                        {
                ...
                case GamePhase.Iterrogations:
                    ...
                    {
                        if ((false == myIsAlien) && (false == myInterogationsCompleted))
                        {
                ...
                case GamePhase.ImplantRemoval:
                    ...
                    {
                        if ((false == myIsAlien) && (false == myImplateRemovalsCompleted))
                        {
        ...
        #region void UpdateCanvas(IGameInstance gi, bool isOnlyLastLegRemoved=false)
        private void UpdateCanvas(IGameInstance gi, bool isOnlyLastLegRemoved = false)
        {
            ...
            // Update the Canvas with new MapItem locations
        ...
        #region bool IsMoveStoppedByAlienBeforeStarted(IGameInstance gi)
        ...
        #region void MovePathDisplay(IMapItemMove mim)
        ...
        #region void MovePathAnimate(IMapItemMove mim, IMapItems persons)
        private void MovePathAnimate(IMapItemMove mim, IMapItems persons)
        {
            ...
            List<Stack> stacks = new List<Stack>();
            stacks.AssignPeople(persons, myIsAlien);
            IEnumerable<Stack> results = from stack in stacks
        ...
        #region bool DisplayConversations(IGameInstance gi)
        ...
        #region void DisplayConversation(IGameInstance gi, ITerritory selectedTerritory)
        private void DisplayConversation(IGameInstance gi, ITerritory selectedTerritory)
        {
            ...
            if (null != peopleInStack)
            {
                ...
                {
                    UpdateActionPanel(gi, !myIsAlien);
        ...
        #region void PerformConversation(IGameInstance gi, bool isIgnoreResults)
        private void PerformConversation(IGameInstance gi, bool isIgnoreResults)
        {
            ...
            Logger.Log(LogEnum.LE_CONVERSATIONS, sbstart);
            if ((0 == myLeftMapItemsInActionPanelSelected.Count)||(0 == myRightMapItemsInActionPanelSelected.Count))
            {
        ...
        #region bool DisplayInfluences(IGameInstance gi)
        ...
        #region void DisplayInfluence(IGameInstance gi, ITerritory selectedTerritory)
        ...
        #region void PerformInfluence(IGameInstance gi, bool isIgnoreResults)
        ...
        #region bool DisplayCombats(IGameInstance gi)
        private bool DisplayCombats(IGameInstance gi)
        {
            ...
            foreach (Stack stack in stacks)
            {
                ...
                if (true == myIsAlien)
                {
                    if (0 == aliens.Count)
                        continue;
                    combatTerritory = aliens[0].Territory;
        ...
        #region void DisplayCombat(IGameInstance gi, ITerritory selectedTerritory)
        private void DisplayCombat(IGameInstance gi, ITerritory selectedTerritory)
        {
            ...
            {
                if ((selectedTerritory.Name == mi.Territory.Name) && (selectedTerritory.Sector == mi.Territory.Sector))
                {
        ...
        #region void PerformCombat(IGameInstance gi, bool isIgnoreResults)
        private void PerformCombat(IGameInstance gi, bool isIgnoreResults)
        {
            ...
            if ((false == myIsCombatInitiatedForAlien) && (false == myIsCombatInitiatedForTownsperson))
            {
                ...
                gi.MapItemCombat.Territory = leftMapItem.Territory;
                if (true == myIsAlien)
                {
        ...
        #region void DisplayCombatResults(IGameInstance gi)
        private void DisplayCombatResults(IGameInstance gi)
        {
            ...
            Logger.Log(LogEnum.LE_COMBAT_THREAD, "DisplayCombatResults() myTextBoxResults.Visibility = Visibility.Visible");
            ...
            Logger.Log(LogEnum.LE_COMBAT_THREAD, "DisplayCombatResults() myTextBoxResults.Text=displayResults");
            myTextBoxResults.Text = displayResults.ToString();
            myLabelLeftTop.Visibility = Visibility.Visible;
            myLabelRightTop.Visibility = Visibility.Visible;
            myTextBoxResults.Visibility = Visibility.Visible;
            Logger.Log(LogEnum.LE_COMBAT_THREAD, "DisplayCombatResults() myTextBoxResults.Text=" + myTextBoxResults.Text);
            UpdateActionPanelButtons(gi);
            Logger.Log(LogEnum.LE_COMBAT_THREAD, "DisplayCombatResults() myTextBoxResults.Text=" + myTextBoxResults.Text);
        }
        #endregion

        #region void PerformCombatRetreat(IGameInstance gi, bool isIgnoreResults)
        private void PerformCombatRetreat(IGameInstance gi, bool isIgnoreResults)
        {
            ...
            UpdateViewState(gi);
            myIsCombatInitiatedForTownsperson = false;
            StringBuilder sb1 = new StringBuilder("UpdateView():TownspersonPerformCombat: "); sb1.Append(myIsAlien.ToString()); sb1.Append("myIsCombatInitiatedForTownsperson=false");
            Logger.Log(LogEnum.LE_COMBAT_STATE, sb1.ToString());
        ...
        #region bool DisplayIterogations(IGameInstance gi)
        ...
        #region bool DisplayImplantRemovals(IGameInstance gi)
        private bool DisplayImplantRemovals(IGameInstance gi)
        {
            ...
            foreach (Stack stack in stacks)
            {
                ...
                String targetName = controlled[0].Territory.Name + controlled[0].Territory.Sector.ToString();
                foreach (UIElement ui in myCanvas.Children)
                {
                    if (ui is Polygon)
                    {
        ...
        #region void DisplayImplantRemoval(IGameInstance gi, ITerritory selectedTerritory)
        ...
        #region void PerformImplantRemoval(IGameInstance gi, bool isIgnoreResults)
        ...
        #region bool DisplayTakeovers(IGameInstance gi, ITerritory selectedTerritory)
        private bool DisplayTakeovers(IGameInstance gi, ITerritory selectedTerritory = null)
        {
            ...
            foreach (Stack stack in stacks)
            {
                ...
                String targetName = possibleVictums[0].Territory.Name + possibleVictums[0].Territory.Sector.ToString();
                foreach (UIElement ui in myCanvas.Children)
                {
                    if (ui is Polygon)
                    {
        ...
        #region void DisplayTakover(IGameInstance gi, ITerritory selectedTerritory)
        ...
        #region PerformTakeover(IGameInstance gi, bool isIgnoreResults)
        ...
        #region void PerformTakeoverObserved(IGameInstance gi)
        ...
        #region void myTextBoxEntryTextChanged(object sender, TextChangedEventArgs e)
        ...
        #region void ClickMapItem(object sender, RoutedEventArgs e)
        private void ClickMapItem(object sender, RoutedEventArgs e)
        {
            ...
            if (sender is Button)
            {
                ...
                {
                    this.RotateStack(selectedMapItem.Territory); // rotate the stack
                    return;
                }
                ...
                switch (gi.GamePhase)                   
                {
                    ...
                    case GamePhase.TownspersonMovement:
                        if( null==myMovingButton )
                        {
                            if ((("Townsperson Selects Counter to Move" != gi.NextAction) || (true == myIsAlien) 
                    ...
                    default:
                        MapItemCommonAction(selectedMapItem.Territory);
                        return;
                }
                ...
                else
                {
                    ...
                    {
                        MapItemMoveManually(selectedMapItem.Territory, myMovingButton);
                    }
                }
            }
        }
        ...
        #region void MouseLeftButtonDownCanvas(object sender, MouseButtonEventArgs e)
        private void MouseLeftButtonDownCanvas(object sender, MouseButtonEventArgs e)
        {
            ...
            switch (gi.GamePhase)
            {
                ...
                case GamePhase.AlienMovement:
                    if ((true == myIsAlien) && (null != myMovingButton))
                ...
                case GamePhase.TownspersonMovement:
                    if ((false == myIsAlien) && (null != myMovingButton))
                        MapItemMoveManually(selectedTerritory, myMovingButton);
                    else
                        RotateStack(selectedTerritory);
                    break;
        ...
        #region void MouseRightButtonDownCanvas(object sender, MouseButtonEventArgs e)
        ...
        #region void MouseDoubleClickMapItem(object sender, RoutedEventArgs  e)
        ...
        #region void ContextMenuLoaded(object sender, RoutedEventArgs e)
                ...
                if (cm.PlacementTarget is Button)
                {
                    ...
                    if ((0 < cm.Items.Count) && (true == mi.IsMoveAllowedToResetThisTurn) )
                    {
                        if (cm.Items[0] is MenuItem)
                        {
                            ...
                            if( (true==myIsAlien) && (GamePhase.AlienMovement==gi.GamePhase)&&(true==mi.IsMoved) )
                                menuItem.IsEnabled = true;
                            else if ((false == myIsAlien) && (GamePhase.TownspersonMovement == gi.GamePhase) && (true == mi.IsMoved))
                                menuItem.IsEnabled = true;
                        }
                    }
                    ...
                    if (1 < cm.Items.Count)
                    {
                        if (cm.Items[1] is MenuItem)
                        {
                            ...
                            List<Stack> stacks = new List<Stack>();
                            stacks.AssignPeople(gi.Persons, myIsAlien);
                            IMapItems mapItems = stacks.FindPeople(mi.Territory);
                            if (null != mapItems)
                            {
                                if( 1 < mapItems.Count )
                                    menuItem.IsEnabled = true;
        ...
        #region void ContextMenuClickReturnToStart(object sender, RoutedEventArgs e)
        ...
        #region void ContextMenuClickRotate(object sender, RoutedEventArgs e)
            ...
            if (sender is MenuItem)
            {
                ...
                if (mi.Parent is ContextMenu)
                {
                    ...
                    if (cm.PlacementTarget is Button)
                    {
                        ...
                        this.RotateStack(selectedMapItem.Territory);
                    }
                }
            }
        }
        ...
        #region void ContextMenuClickExposeAlien(object sender, RoutedEventArgs e)
        ...
        #region void ContextMenuClickStopMove(object sender, RoutedEventArgs e)
                        ...
                        if (null != selectedMapItem)
                        {
                            if (((true == selectedMapItem.IsAlienUnknown) || (true == selectedMapItem.IsAlienKnown)) && (true == myIsAlienAbleToStopMove) && (false == selectedMapItem.IsMoveStoppedThisTurn))
                            {
                                ...
                                if (0 < gi.MapItemMoves.Count)
                                {
                                    ...
                                    movingMi.Territory = movingMi.TerritoryStarting;
        ...
        #region static void TimerElasped(object sender, ElapsedEventArgs  e)
        ...
        #region void myButton1_Click(object sender, RoutedEventArgs e)
        ...
        #region void myButton2_Click(object sender, RoutedEventArgs e)
        ...
        #region void myButton3_Click(object sender, RoutedEventArgs e)
        ...
        #region void myButton4_Click(object sender, RoutedEventArgs e)
        ...
        #region void myButton5_Click(object sender, RoutedEventArgs e)
        ...
        #region void myButton6_Click(object sender, RoutedEventArgs e)
        ...
        #region void myButtonOk_Click(object sender, RoutedEventArgs e)
        ...
        #region void myButtonIgnoreClick(object sender, RoutedEventArgs e)
        ...
        #region void GameViewerWindowClosed()
        ...
        #region void MapItemCommonAction(ITerritory selectedTerritory)
        private void MapItemCommonAction(ITerritory selectedTerritory)
        {
            ...
            switch (gi.GamePhase)
            {
                #region GamePhase.Conversations
                case GamePhase.Conversations:
                    if (false == myIsAlien)
                ...
                #region GamePhase.Combat
                case GamePhase.Combat:
                    DisplayCombat(gi, selectedTerritory);
                    return;
                #endregion

                #region GamePhase.Iterrogations
                case GamePhase.Iterrogations:
                    if (false == myIsAlien)
                    {
                        if ((true == selectedTerritory.IsBuilding) && (null != gi.ZebulonTerritories.Find(selectedTerritory.ToString())) && (0 < gi.NumIterogationsThisTurn))
                        {
                            ...
                            if ((zebulon.Territory.Name == selectedTerritory.Name) && (zebulon.Territory.Sector == selectedTerritory.Sector))
                            {
                                zebulon.IsAlienKnown = true;
                ...
                #region GamePhase.ImplantRemoval
                case GamePhase.ImplantRemoval:
                    if (false == myIsAlien)
                ...
                #region GamePhase.AlienTakeover
                case GamePhase.AlienTakeover:
                    if (true == myIsAlien)
        ...
        #region void MapItemMoveManually(ITerritory selectedTerritory, Button selectedButton)
        private void MapItemMoveManually(ITerritory selectedTerritory, Button selectedButton)
        {
            ...
            if ((null != selectedTerritory) && (null != selectedButton))
            {
                ...
                if ((selectedTerritory.Name == movingMapItem.Territory.Name) && (selectedTerritory.Sector == movingMapItem.Territory.Sector))
                {
                    this.RotateStack(selectedTerritory); // rotate the stack
                }
                ...
                else
                {
                    ...
                    movingMapItem.TerritoryStarting = movingMapItem.Territory;
                    MapItemMove mim = new MapItemMove(movingMapItem, selectedTerritory, gi.Persons);
        ...
        #region bool IsAlienAbleToStopMove(IGameInstance gi, IMapItem mi)
        private bool IsAlienAbleToStopMove(IGameInstance gi, IMapItem mi)
        {
             ...
             && (false == mi.IsMoveStoppedThisTurn) && (GamePhase.TownspersonMovement == gi.GamePhase))
            {
                if (0 < gi.MapItemMoves.Count)
                {
                    ...
                    IMapItem movingMI = gi.Persons.Find(mim.MapItem.Name);
                    if ((mi.Territory.Name == mim.OldTerritory.Name) && (mi.Territory.Sector == mim.OldTerritory.Sector))
                    {
                    ...
                    else
                    {
                        foreach (ITerritory t in mim.BestPath.Territories)
                        {
                            if ((mi.Territory.Name == t.Name) && (mi.Territory.Sector == t.Sector))
                            {
                                if ((true == movingMI.IsControlled) && (false == movingMI.IsStunned) && (false == movingMI.IsTiedUp)
                                     && (false == movingMI.IsSurrendered) && (false == movingMI.IsStunned) && (false == movingMI.IsKilled)
        ...
        #region void MapItemReturnToStart(Button selectedButton)
        private void MapItemReturnToStart(Button selectedButton)
        {
            ...
            {
                if ((true == myIsFlagSetForMoveReset)&&(true==myIsAlien)&&(GamePhase.AlienMovement == gi.GamePhase) )
                    MessageBox.Show("Reset Not Allowed");
                if ((true == myIsFlagSetForMoveReset) && (false == myIsAlien) && (GamePhase.TownspersonMovement == gi.GamePhase))
                    MessageBox.Show("Reset Not Allowed");
            ...
            {
                case GamePhase.AlienMovement:
                    if ((true == selectedMapItem.IsControlled) || (false == myIsAlien))
                    {
                        this.RotateStack(selectedMapItem.Territory); // rotate the stack
                        return;  // do nothing
                    }
                ...
                case GamePhase.TownspersonMovement:
                    if ((false == selectedMapItem.IsControlled) || (true == myIsAlien))
                    {
                        this.RotateStack(selectedMapItem.Territory); // rotate the stack
                        return;  // do nothing
                    }
            ...
            StringBuilder sb = new StringBuilder("MapItemReturnToStart(): t=");sb.Append(selectedMapItem.Territory.ToString());sb.Append(" st=");sb.Append(selectedMapItem.TerritoryStarting.ToString());
            Logger.Log( LogEnum.LE_RETURN_TO_START, sb.ToString());

            if (selectedMapItem.Territory != selectedMapItem.TerritoryStarting)
            {
                ...
                if( 0 < gi.MapItemMoves.Count )
                {
                    ...
                    {
                        previousMovingMi1.Territory = previousMovingMi1.TerritoryStarting;
        ...
        private void RotateStack(ITerritory selectedTerritory)
        {
            ...
            // Remove the bottom MapItem, Bounding Rectable, and button.

            Rectangle bottomRect = null;
            IMapItem bottomMi = gi.Persons.Find(alivePeopleInStack[0].Name);
            if (null == bottomMi)
                return;
            ...
            // Shift the remaining MapItems in the stack
            ...
            // Add back the bottom items to the top

            gi.Persons.Add(bottomMi);
            bottomMi.Location = new MapPoint(bottomMi.Territory.CenterPoint.X - Utilities.theXOffset + (counterCount * 3), bottomMi.Territory.CenterPoint.Y - Utilities.theYOffset + (counterCount * 3));
            ...
            MapItem.SetButtonContent(bottomButton, bottomMi, myIsAlien);
            if (null != bottomRect)
            {
